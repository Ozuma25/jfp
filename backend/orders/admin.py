...
class OrderAdmin(admin.ModelAdmin):
    ...
    search_fields = ("user__email", "shipping_name", "razorpay_order_id")
    inlines = (OrderLineInline, OrderStatusHistoryInline)
    fieldsets = (
        (None, {"fields": ("user", "status", "currency", "subtotal", "total")}),
        ...
        if change:
            ...
            if old_obj.status != obj.status:
                ...
                if obj.status == Order.Status.DESIGN_REJECTED:
                    note = f"Rejected: {obj.admin_rejection_reason}"
                elif obj.status == Order.Status.DESIGN_APPROVED_PENDING_PAYMENT:
                    note = "Bespoke design approved. Awaiting payment."
