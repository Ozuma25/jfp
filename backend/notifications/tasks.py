...
def send_email_async(self, subject, message, recipient_list, from_email=None, html_message=None):
    """
    Generic task to send emails asynchronously using Celery.
    """
    if not from_email:
        from_email = getattr(settings, "DEFAULT_FROM_EMAIL", "noreply@jaifancypacks.com")
    
    try:
        logger.info(f"Sending email async: '{subject}' to {recipient_list}")
        send_mail(
            subject,
            message,
            from_email,
...
def check_abandoned_carts():
    """
    Periodic task to nudge users who left items in their cart.
    Threshold: 2 hours of inactivity.
    """
    cutoff = timezone.now() - timedelta(hours=2)
    # Only target authenticated users with at least one item
    ...
    for cart in abandoned_carts:
        ...
        send_email_async.delay(subject, message, [user.email], html_message=html_message)
