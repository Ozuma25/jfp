...
export function WishlistButton({ productId, variant = "icon" }: Props) {
  ...
  useEffect(() => {
    ...
    const checkWishlist = async () => {
      try {
        ...
        const wlUrl = await resolveApiFetchUrl("/api/wishlist/");
  ...
  const toggleWishlist = async (e: React.MouseEvent) => {
    ...
    try {
      if (isInWishlist) {
        const rmUrl = await resolveApiFetchUrl(`/api/wishlist/remove/${productId}/`);
        ...
        const addUrl = await resolveApiFetchUrl("/api/wishlist/");
