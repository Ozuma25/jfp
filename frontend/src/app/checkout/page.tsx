  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Sign in to checkout securely</h1>
        <p className="mb-8 text-neutral-600">You must be logged into an authorized account to complete purchases.</p>
        <Link href="/login?next=/checkout" className="inline-block bg-[#F0C75E] text-black font-semibold rounded-md px-10 py-3 hover:bg-[#D4AF37] transition-colors shadow-sm">
          Sign in securely
        </Link>
      </div>
    );
                    ...
                    {savedAddresses.length > 0 && !useNewAddress ? (
                      ...
                      <form onSubmit={handleAddressSubmit} className="space-y-4 max-w-lg">
                        ...
                        <div>
                          ...
                          <input required type="tel" value={shipping_phone} onChange={(e) => setShippingPhone(e.target.value)} className="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-store-yellow focus:ring-1 focus:ring-store-yellow focus:outline-none shadow-sm" />
                        </div>
                        <div>
