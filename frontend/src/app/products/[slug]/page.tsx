  ...
  return (
    <div className="bg-white min-h-screen pb-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 md:py-10">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 lg:gap-14">
          ...
          <div className="w-full lg:w-[50%] xl:w-[55%]">
            <div className="lg:sticky lg:top-24">
              ...
              <div className="mt-12 hidden lg:block bg-neutral-50 px-8 py-10 rounded-2xl border border-gray-100">
                <h2 className="text-xl font-serif text-store-navy mb-6">The Product Story</h2>
          ...
          <div className="w-full lg:w-[45%] xl:w-[40%]">
            <div className="space-y-8">
                ...
                {product.review_count > 0 && (
                  <div className="flex items-center gap-4 text-store-button text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-store-navy leading-none">{Number(product.rating).toFixed(1)}</span>
                    ...
                    <span className="text-neutral-400 font-bold uppercase tracking-widest text-[9px]">
                      {product.review_count} Verified {product.review_count === 1 ? 'Review' : 'Reviews'}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="pb-6 border-b border-gray-100">
                <div className="flex items-baseline gap-3 mb-1">
                  ...
                  {product.compare_at_price_display && (
                    <span className="text-base text-neutral-400 line-through decoration-store-button/40">{product.compare_at_price_display}</span>
                  )}
                </div>
                ...
                {product.recent_sales_count > 0 && (
                  <div className="flex items-center gap-2 bg-[#F9F9F7] px-3 py-1.5 rounded-md border border-gray-100">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-store-navy">
                      <span className="text-store-button bg-store-button/10 px-1 rounded">{product.recent_sales_count} people</span> bought this in the last 24h
                    </p>
                  </div>
                )}
                {product.stock > 0 && product.stock <= 10 && (
                  <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-md border border-red-100">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-red-600">
                      Only <span className="bg-red-100 px-1 rounded">{product.stock} left</span> — Order soon
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-center gap-6 py-4 border-t border-gray-100">
                {/* Secure - Navy Blue */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#004B6E]/5 hover:bg-[#004B6E]/10 transition-all hover:scale-105 cursor-default">
                  <span className="text-lg">🛡️</span>
                  <span className="text-[8px] font-bold uppercase tracking-widest whitespace-nowrap text-[#004B6E]">Secure</span>
                </div>
                {/* Premium - Gold */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 transition-all hover:scale-105 cursor-default">
                  <span className="text-lg">🌿</span>
                  <span className="text-[8px] font-bold uppercase tracking-widest whitespace-nowrap text-[#D4AF37]">Premium</span>
                </div>
                {/* Pride - Tricolor */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-orange-50 hover:bg-orange-100 transition-all hover:scale-105 cursor-default">
                  <span className="text-lg">🇮🇳</span>
                  <span className="text-[8px] font-bold uppercase tracking-widest whitespace-nowrap text-orange-600">Pride</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-12 lg:hidden bg-neutral-50 p-6 rounded-2xl border border-gray-100">
          <h2 className="text-xl font-serif text-store-navy mb-4">The Product Story</h2>
          <div className="prose prose-sm prose-neutral max-w-none text-neutral-600 leading-relaxed">
            {product.description.split('\n').filter(Boolean).map((line, i) => (
              <p key={i} className="mb-4">{line}</p>
            ))}
          ...
          <div className="mt-20 pt-16 border-t border-gray-100">
            <div className="flex items-end justify-between mb-8">
              <div>
                ...
                <p className="mt-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400">Curated pieces from the {product.category_slug.replace('-', ' ')} collection</p>
              </div>
