...
export default function AddressesPage() {
  ...
  const handlePincodeChange = async (val: string) => {
    ...
    setFormData(prev => ({ ...prev, postal_code: cleaned }));
    if (cleaned.length === 6) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.name.trim() || !formData.recipient_name.trim() || !formData.address_line1.trim() || !formData.city.trim() || !formData.state.trim()) {
    ...
    const cleanPincode = formData.postal_code.replace(/\D/g, "");
    ...
    try {
      if (isEditing === "new") {
        ...
        await updateAddress(isEditing, payload);
      }
      setIsEditing(null);
      await load();
    } catch (err) {
