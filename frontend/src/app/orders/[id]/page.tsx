...
import { getApiBase, resolveApiFetchUrl } from "@/lib/api";
...
function ItemReviewForm({ productId, orderId, productTitle, onSuccess }: { productId: number, orderId: number, productTitle: string, onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const revUrl = await resolveApiFetchUrl("/api/reviews/");
...
export default function OrderDetailPage() {
  ...
  const isNewSuccess = searchParams.get("success") === "1";

  async function handleDownloadReceipt() {
    setBusy(true); setActionErr("");
    try {
      const token = getAccessToken();
      const rcUrl = await resolveApiFetchUrl(`/api/orders/${id}/receipt/`);
  ...
  return (
    <div className="order-details-modern animate-in fade-in">
      <div className="container-custom py-5">
        ...
        {isNewSuccess && (
          <div className="bg-green-50 border border-green-200 text-green-800 px-6 py-4 rounded-xl mb-6 shadow-sm flex items-center gap-3 animate-in slide-in-from-top-4">
            <span className="text-2xl">🎉</span>
            <div>
              <h4 className="font-bold text-lg mb-0">Order Placed Successfully!</h4>
              <p className="text-sm mb-0">Thank you for your purchase. We are processing your order right away.</p>
            </div>
            <ConfettiEffect />
          </div>
        )}
